...
type PartialUEContextTransferFailure struct {
...
func (msg *PartialUEContextTransferFailure) toIEs() []XnAPMessageIE {
...
func (msg *PartialUEContextTransferFailure) Encode(w io.Writer) error {
...
func (msg *PartialUEContextTransferFailure) Decode(wire []byte) error {
	d := per.NewDecoder(wire, per.APER)

	seq := d.NewSequenceDecoder(per.SequenceConstraints{Extensible: true})
	if err := seq.DecodeExtensionBit(); err != nil {
		return fmt.Errorf("decode PartialUEContextTransferFailure extension bit: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("decode PartialUEContextTransferFailure protocolIEs length: %w", err)
	}
	...
	for i := range n {
		...
		if err != nil {
			return fmt.Errorf("decode PartialUEContextTransferFailure IE[%d] id: %w", i, err)
		}
		seen[id] = true

		var criticality Criticality
		if err := criticality.Decode(d); err != nil {
			return fmt.Errorf("decode PartialUEContextTransferFailure IE[%d] criticality: %w", i, err)
		}

		payload, err := d.DecodeOpenType()
		if err != nil {
			return fmt.Errorf("decode PartialUEContextTransferFailure IE[%d] value: %w", i, err)
		}
		...
		default:
			if criticality.Value == CriticalityReject {
				return fmt.Errorf("unsupported PartialUEContextTransferFailure IE id=%d with reject criticality", id)
			}
		}
		if err != nil {
			return fmt.Errorf("decode PartialUEContextTransferFailure IE id=%d: %w", id, err)
		}
	}

	if _, err := seq.DecodeExtensionAdditions(); err != nil {
		return fmt.Errorf("decode PartialUEContextTransferFailure extensions: %w", err)
	}
	...
	for _, id := range []int64{
		...
		if !seen[id] {
			return fmt.Errorf("missing mandatory PartialUEContextTransferFailure IE id=%d", id)
		}
	}
