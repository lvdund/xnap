...
type RetrieveUEContextFailure struct {
	...
	OldtoNewNGRANnodeResumeContainer []byte
	Cause                            Cause
	CriticalityDiagnostics           *CriticalityDiagnostics
}

func (msg *RetrieveUEContextFailure) toIEs() []XnAPMessageIE {
	...
	if len(msg.OldtoNewNGRANnodeResumeContainer) > 0 {
		ies = append(ies, XnAPMessageIE{
			ID:          ProtocolIEID{Value: int64(common.ProtocolIEID_OldtoNewNGRANnodeResumeContainer)},
			Criticality: Criticality{Value: CriticalityIgnore},
			Value:       &octetStringIE{Value: msg.OldtoNewNGRANnodeResumeContainer},
		})
	}
	ies = append(ies, XnAPMessageIE{
...
func (msg *RetrieveUEContextFailure) Encode(w io.Writer) error {
...
func (msg *RetrieveUEContextFailure) Decode(wire []byte) error {
	d := per.NewDecoder(wire, per.APER)

	seq := d.NewSequenceDecoder(per.SequenceConstraints{Extensible: true})
	if err := seq.DecodeExtensionBit(); err != nil {
		return fmt.Errorf("decode RetrieveUEContextFailure extension bit: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("decode RetrieveUEContextFailure protocolIEs length: %w", err)
	}
	...
	for i := range n {
		...
		if err != nil {
			return fmt.Errorf("decode RetrieveUEContextFailure IE[%d] id: %w", i, err)
		}
		seen[id] = true

		var criticality Criticality
		if err := criticality.Decode(d); err != nil {
			return fmt.Errorf("decode RetrieveUEContextFailure IE[%d] criticality: %w", i, err)
		}

		payload, err := d.DecodeOpenType()
		if err != nil {
			return fmt.Errorf("decode RetrieveUEContextFailure IE[%d] value: %w", i, err)
		}
		...
		case common.ProtocolIEID_OldtoNewNGRANnodeResumeContainer:
			v := octetStringIE{}
			err = v.Decode(ies)
			msg.OldtoNewNGRANnodeResumeContainer = v.Value
		...
		default:
			if criticality.Value == CriticalityReject {
				return fmt.Errorf("unsupported RetrieveUEContextFailure IE id=%d with reject criticality", id)
			}
		}
		if err != nil {
			return fmt.Errorf("decode RetrieveUEContextFailure IE id=%d: %w", id, err)
		}
	}

	if _, err := seq.DecodeExtensionAdditions(); err != nil {
		return fmt.Errorf("decode RetrieveUEContextFailure extensions: %w", err)
	}
	...
	for _, id := range []int64{
		...
		if !seen[id] {
			return fmt.Errorf("missing mandatory RetrieveUEContextFailure IE id=%d", id)
		}
	}
