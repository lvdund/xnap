...
var lTEA2XServicesAuthorizedConstraints = per.SequenceConstraints{
	Extensible: true,
...
type LTEA2XServicesAuthorized struct {
	AerialUE           *AerialUE
	AerialControllerUE *AerialControllerUE
	IEExtensions       []byte
}

func (ie *LTEA2XServicesAuthorized) Encode(e *per.Encoder) error {
	seq := e.NewSequenceEncoder(lTEA2XServicesAuthorizedConstraints)
...
func (ie *LTEA2XServicesAuthorized) Decode(d *per.Decoder) error {
	seq := d.NewSequenceDecoder(lTEA2XServicesAuthorizedConstraints)
