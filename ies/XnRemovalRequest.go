...
type XnRemovalRequest struct {
	...
	XnRemovalThreshold          *XnBenefitValue
	InterfaceInstanceIndication *InterfaceInstanceIndication
}

func (msg *XnRemovalRequest) toIEs() []XnAPMessageIE {
	...
	if msg.XnRemovalThreshold != nil {
		ies = append(ies, XnAPMessageIE{
			ID:          ProtocolIEID{Value: int64(common.ProtocolIEID_XnRemovalThreshold)},
			Criticality: Criticality{Value: CriticalityReject},
			Value:       msg.XnRemovalThreshold,
		})
	}
...
func (msg *XnRemovalRequest) Encode(w io.Writer) error {
	...
	if err := procedureCodeXnMsg(outer, common.ProcedureCode_XnRemoval); err != nil {
		return err
	}
...
func (msg *XnRemovalRequest) Decode(wire []byte) error {
	d := per.NewDecoder(wire, per.APER)

	seq := d.NewSequenceDecoder(per.SequenceConstraints{Extensible: true})
	if err := seq.DecodeExtensionBit(); err != nil {
		return fmt.Errorf("decode XnRemovalRequest extension bit: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("decode XnRemovalRequest protocolIEs length: %w", err)
	}
	...
	for i := range n {
		...
		if err != nil {
			return fmt.Errorf("decode XnRemovalRequest IE[%d] id: %w", i, err)
		}
		seen[id] = true

		var criticality Criticality
		if err := criticality.Decode(d); err != nil {
			return fmt.Errorf("decode XnRemovalRequest IE[%d] criticality: %w", i, err)
		}

		payload, err := d.DecodeOpenType()
		if err != nil {
			return fmt.Errorf("decode XnRemovalRequest IE[%d] value: %w", i, err)
		}
		...
		case common.ProtocolIEID_XnRemovalThreshold:
			v := &XnBenefitValue{}
			err = v.Decode(ies)
			msg.XnRemovalThreshold = v
		...
		default:
			if criticality.Value == CriticalityReject {
				return fmt.Errorf("unsupported XnRemovalRequest IE id=%d with reject criticality", id)
			}
		}
		if err != nil {
			return fmt.Errorf("decode XnRemovalRequest IE id=%d: %w", id, err)
		}
	}

	if _, err := seq.DecodeExtensionAdditions(); err != nil {
		return fmt.Errorf("decode XnRemovalRequest extensions: %w", err)
	}
	...
	for _, id := range []int64{
		...
		if !seen[id] {
			return fmt.Errorf("missing mandatory XnRemovalRequest IE id=%d", id)
		}
	}
