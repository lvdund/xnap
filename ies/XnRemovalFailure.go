...
func (msg *XnRemovalFailure) toIEs() []XnAPMessageIE {
...
func (msg *XnRemovalFailure) Encode(w io.Writer) error {
...
func (msg *XnRemovalFailure) Decode(wire []byte) error {
	d := per.NewDecoder(wire, per.APER)

	seq := d.NewSequenceDecoder(per.SequenceConstraints{Extensible: true})
	if err := seq.DecodeExtensionBit(); err != nil {
		return fmt.Errorf("decode XnRemovalFailure extension bit: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("decode XnRemovalFailure protocolIEs length: %w", err)
	}
	...
	for i := range n {
		...
		if err != nil {
			return fmt.Errorf("decode XnRemovalFailure IE[%d] id: %w", i, err)
		}
		seen[id] = true

		var criticality Criticality
		if err := criticality.Decode(d); err != nil {
			return fmt.Errorf("decode XnRemovalFailure IE[%d] criticality: %w", i, err)
		}

		payload, err := d.DecodeOpenType()
		if err != nil {
			return fmt.Errorf("decode XnRemovalFailure IE[%d] value: %w", i, err)
		}
		...
		default:
			if criticality.Value == CriticalityReject {
				return fmt.Errorf("unsupported XnRemovalFailure IE id=%d with reject criticality", id)
			}
		}
		if err != nil {
			return fmt.Errorf("decode XnRemovalFailure IE id=%d: %w", id, err)
		}
	}

	if _, err := seq.DecodeExtensionAdditions(); err != nil {
		return fmt.Errorf("decode XnRemovalFailure extensions: %w", err)
	}
	...
	for _, id := range []int64{
		...
		if !seen[id] {
			return fmt.Errorf("missing mandatory XnRemovalFailure IE id=%d", id)
		}
	}
