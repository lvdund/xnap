...
type ResetRequest struct {
	ResetRequestTypeInfo        ResetRequestTypeInfo
	Cause                       Cause
	InterfaceInstanceIndication *InterfaceInstanceIndication
}

func (msg *ResetRequest) toIEs() []XnAPMessageIE {
	ies := make([]XnAPMessageIE, 0)
	ies = append(ies, XnAPMessageIE{
		ID:          ProtocolIEID{Value: int64(common.ProtocolIEID_ResetRequestTypeInfo)},
		Criticality: Criticality{Value: CriticalityReject},
		Value:       &msg.ResetRequestTypeInfo,
	})
	ies = append(ies, XnAPMessageIE{
...
func (msg *ResetRequest) Encode(w io.Writer) error {
...
func (msg *ResetRequest) Decode(wire []byte) error {
	d := per.NewDecoder(wire, per.APER)

	seq := d.NewSequenceDecoder(per.SequenceConstraints{Extensible: true})
	if err := seq.DecodeExtensionBit(); err != nil {
		return fmt.Errorf("decode ResetRequest extension bit: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("decode ResetRequest protocolIEs length: %w", err)
	}
	...
	for i := range n {
		...
		if err != nil {
			return fmt.Errorf("decode ResetRequest IE[%d] id: %w", i, err)
		}
		seen[id] = true

		var criticality Criticality
		if err := criticality.Decode(d); err != nil {
			return fmt.Errorf("decode ResetRequest IE[%d] criticality: %w", i, err)
		}

		payload, err := d.DecodeOpenType()
		if err != nil {
			return fmt.Errorf("decode ResetRequest IE[%d] value: %w", i, err)
		}
		ies := per.NewDecoder(payload, per.APER)

		switch id {
		case common.ProtocolIEID_ResetRequestTypeInfo:
			err = msg.ResetRequestTypeInfo.Decode(ies)
		...
		default:
			if criticality.Value == CriticalityReject {
				return fmt.Errorf("unsupported ResetRequest IE id=%d with reject criticality", id)
			}
		}
		if err != nil {
			return fmt.Errorf("decode ResetRequest IE id=%d: %w", id, err)
		}
	}

	if _, err := seq.DecodeExtensionAdditions(); err != nil {
		return fmt.Errorf("decode ResetRequest extensions: %w", err)
	}

	for _, id := range []int64{
		common.ProtocolIEID_ResetRequestTypeInfo,
		common.ProtocolIEID_Cause,
	} {
		if !seen[id] {
			return fmt.Errorf("missing mandatory ResetRequest IE id=%d", id)
		}
	}
