...
var trafficToBeAddedItemConstraints = per.SequenceConstraints{
	Extensible: true,
...
type TrafficToBeAddedItem struct {
...
func (ie *TrafficToBeAddedItem) Encode(e *per.Encoder) error {
	seq := e.NewSequenceEncoder(trafficToBeAddedItemConstraints)
	hasExt := len(ie.IEExtensions) > 0
	if err := seq.EncodeExtensionBit(hasExt); err != nil {
		return err
	}
	if err := seq.EncodePreamble([]bool{ie.F1TerminatingTopologyBHInformation != nil, false}); err != nil {
		return err
	}
...
func (ie *TrafficToBeAddedItem) Decode(d *per.Decoder) error {
	seq := d.NewSequenceDecoder(trafficToBeAddedItemConstraints)
