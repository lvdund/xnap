...
var trafficModifiedItemConstraints = per.SequenceConstraints{
	Extensible: true,
...
type TrafficModifiedItem struct {
	TrafficIndex                          TrafficIndex
	NonF1TerminatingTopologyBHInformation NonF1TerminatingTopologyBHInformation
	IEExtensions                          []byte
}

func (ie *TrafficModifiedItem) Encode(e *per.Encoder) error {
	seq := e.NewSequenceEncoder(trafficModifiedItemConstraints)
...
func (ie *TrafficModifiedItem) Decode(d *per.Decoder) error {
	seq := d.NewSequenceDecoder(trafficModifiedItemConstraints)
