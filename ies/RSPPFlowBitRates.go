...
var rSPPFlowBitRatesConstraints = per.SequenceConstraints{
	Extensible: true,
...
type RSPPFlowBitRates struct {
	GuaranteedFlowBitRate BitRate
	MaximumFlowBitRate    BitRate
	IEExtensions          []byte
}

func (ie *RSPPFlowBitRates) Encode(e *per.Encoder) error {
	seq := e.NewSequenceEncoder(rSPPFlowBitRatesConstraints)
...
func (ie *RSPPFlowBitRates) Decode(d *per.Decoder) error {
	seq := d.NewSequenceDecoder(rSPPFlowBitRatesConstraints)
