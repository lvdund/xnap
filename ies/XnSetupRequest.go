...
func (msg *XnSetupRequest) toIEs() []XnAPMessageIE {
...
func (msg *XnSetupRequest) Encode(w io.Writer) error {
...
func (msg *XnSetupRequest) Decode(wire []byte) error {
	d := per.NewDecoder(wire, per.APER)

	seq := d.NewSequenceDecoder(per.SequenceConstraints{Extensible: true})
	if err := seq.DecodeExtensionBit(); err != nil {
		return fmt.Errorf("decode XnSetupRequest extension bit: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("decode XnSetupRequest protocolIEs length: %w", err)
	}
	...
	for i := range n {
		...
		if err != nil {
			return fmt.Errorf("decode XnSetupRequest IE[%d] id: %w", i, err)
		}
		seen[id] = true

		var criticality Criticality
		if err := criticality.Decode(d); err != nil {
			return fmt.Errorf("decode XnSetupRequest IE[%d] criticality: %w", i, err)
		}

		payload, err := d.DecodeOpenType()
		if err != nil {
			return fmt.Errorf("decode XnSetupRequest IE[%d] value: %w", i, err)
		}
		...
		case common.ProtocolIEID_AMFRegionInformation:
			err = msg.AMFRegionInformation.Decode(ies)
		...
		default:
			if criticality.Value == CriticalityReject {
				return fmt.Errorf("unsupported XnSetupRequest IE id=%d with reject criticality", id)
			}
		}
		if err != nil {
			return fmt.Errorf("decode XnSetupRequest IE id=%d: %w", id, err)
		}
	}

	if _, err := seq.DecodeExtensionAdditions(); err != nil {
		return fmt.Errorf("decode XnSetupRequest extensions: %w", err)
	}

	for _, id := range []int64{
		common.ProtocolIEID_GlobalNGRANNodeID,
		common.ProtocolIEID_TAISupportList,
		common.ProtocolIEID_AMFRegionInformation,
	} {
		if !seen[id] {
			return fmt.Errorf("missing mandatory XnSetupRequest IE id=%d", id)
		}
	}
