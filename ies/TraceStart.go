...
func (msg *TraceStart) toIEs() []XnAPMessageIE {
...
func (msg *TraceStart) Encode(w io.Writer) error {
	...
	if err := procedureCodeXnMsg(outer, common.ProcedureCode_TraceStart); err != nil {
		return err
	}
...
func (msg *TraceStart) Decode(wire []byte) error {
	...
	if err != nil {
		return fmt.Errorf("decode TraceStart protocolIEs length: %w", err)
	}
	...
	for i := range n {
		...
		if err != nil {
			return fmt.Errorf("decode TraceStart IE[%d] id: %w", i, err)
		}
		seen[id] = true

		var criticality Criticality
		if err := criticality.Decode(d); err != nil {
			return fmt.Errorf("decode TraceStart IE[%d] criticality: %w", i, err)
		}

		payload, err := d.DecodeOpenType()
		if err != nil {
			return fmt.Errorf("decode TraceStart IE[%d] value: %w", i, err)
		}
		...
		default:
			if criticality.Value == CriticalityReject {
				return fmt.Errorf("unsupported TraceStart IE id=%d with reject criticality", id)
			}
		}
		if err != nil {
			return fmt.Errorf("decode TraceStart IE id=%d: %w", id, err)
		}
	}

	if _, err := seq.DecodeExtensionAdditions(); err != nil {
		return fmt.Errorf("decode TraceStart extensions: %w", err)
	}
	...
	for _, id := range []int64{
		...
		if !seen[id] {
			return fmt.Errorf("missing mandatory TraceStart IE id=%d", id)
		}
	}
