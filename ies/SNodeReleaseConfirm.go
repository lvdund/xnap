...
type SNodeReleaseConfirm struct {
	...
	PDUSessionReleasedListRelConf *PDUSessionReleasedListRelConf
	CriticalityDiagnostics        *CriticalityDiagnostics
}

func (msg *SNodeReleaseConfirm) toIEs() []XnAPMessageIE {
	...
	if msg.PDUSessionReleasedListRelConf != nil {
		ies = append(ies, XnAPMessageIE{
			ID:          ProtocolIEID{Value: int64(common.ProtocolIEID_PDUSessionReleasedListRelConf)},
			Criticality: Criticality{Value: CriticalityIgnore},
			Value:       msg.PDUSessionReleasedListRelConf,
		})
	}
...
func (msg *SNodeReleaseConfirm) Encode(w io.Writer) error {
...
func (msg *SNodeReleaseConfirm) Decode(wire []byte) error {
	d := per.NewDecoder(wire, per.APER)

	seq := d.NewSequenceDecoder(per.SequenceConstraints{Extensible: true})
	if err := seq.DecodeExtensionBit(); err != nil {
		return fmt.Errorf("decode SNodeReleaseConfirm extension bit: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("decode SNodeReleaseConfirm protocolIEs length: %w", err)
	}
	...
	for i := range n {
		...
		if err != nil {
			return fmt.Errorf("decode SNodeReleaseConfirm IE[%d] id: %w", i, err)
		}
		seen[id] = true

		var criticality Criticality
		if err := criticality.Decode(d); err != nil {
			return fmt.Errorf("decode SNodeReleaseConfirm IE[%d] criticality: %w", i, err)
		}

		payload, err := d.DecodeOpenType()
		if err != nil {
			return fmt.Errorf("decode SNodeReleaseConfirm IE[%d] value: %w", i, err)
		}
		...
		case common.ProtocolIEID_PDUSessionReleasedListRelConf:
			v := &PDUSessionReleasedListRelConf{}
			err = v.Decode(ies)
			msg.PDUSessionReleasedListRelConf = v
		...
		default:
			if criticality.Value == CriticalityReject {
				return fmt.Errorf("unsupported SNodeReleaseConfirm IE id=%d with reject criticality", id)
			}
		}
		if err != nil {
			return fmt.Errorf("decode SNodeReleaseConfirm IE id=%d: %w", id, err)
		}
	}

	if _, err := seq.DecodeExtensionAdditions(); err != nil {
		return fmt.Errorf("decode SNodeReleaseConfirm extensions: %w", err)
	}
	...
	for _, id := range []int64{
		...
		if !seen[id] {
			return fmt.Errorf("missing mandatory SNodeReleaseConfirm IE id=%d", id)
		}
	}
