...
type F1CTrafficTransfer struct {
	...
	F1CTrafficContainer F1CTrafficContainer
}

func (msg *F1CTrafficTransfer) toIEs() []XnAPMessageIE {
	...
	ies = append(ies, XnAPMessageIE{
		ID:          ProtocolIEID{Value: int64(common.ProtocolIEID_F1CTrafficContainer)},
		Criticality: Criticality{Value: CriticalityReject},
		Value:       &msg.F1CTrafficContainer,
	})
	return ies
}

func (msg *F1CTrafficTransfer) Encode(w io.Writer) error {
	...
	if err := procedureCodeXnMsg(outer, common.ProcedureCode_F1CTrafficTransfer); err != nil {
		return err
	}
...
func (msg *F1CTrafficTransfer) Decode(wire []byte) error {
	d := per.NewDecoder(wire, per.APER)

	seq := d.NewSequenceDecoder(per.SequenceConstraints{Extensible: true})
	if err := seq.DecodeExtensionBit(); err != nil {
		return fmt.Errorf("decode F1CTrafficTransfer extension bit: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("decode F1CTrafficTransfer protocolIEs length: %w", err)
	}
	...
	for i := range n {
		...
		if err != nil {
			return fmt.Errorf("decode F1CTrafficTransfer IE[%d] id: %w", i, err)
		}
		seen[id] = true

		var criticality Criticality
		if err := criticality.Decode(d); err != nil {
			return fmt.Errorf("decode F1CTrafficTransfer IE[%d] criticality: %w", i, err)
		}

		payload, err := d.DecodeOpenType()
		if err != nil {
			return fmt.Errorf("decode F1CTrafficTransfer IE[%d] value: %w", i, err)
		}
		...
		case common.ProtocolIEID_F1CTrafficContainer:
			err = msg.F1CTrafficContainer.Decode(ies)
		default:
			if criticality.Value == CriticalityReject {
				return fmt.Errorf("unsupported F1CTrafficTransfer IE id=%d with reject criticality", id)
			}
		}
		if err != nil {
			return fmt.Errorf("decode F1CTrafficTransfer IE id=%d: %w", id, err)
		}
	}

	if _, err := seq.DecodeExtensionAdditions(); err != nil {
		return fmt.Errorf("decode F1CTrafficTransfer extensions: %w", err)
	}

	for _, id := range []int64{
		common.ProtocolIEID_MNGRANnodeUEXnAPID,
		common.ProtocolIEID_SNGRANnodeUEXnAPID,
		common.ProtocolIEID_F1CTrafficContainer,
	} {
		if !seen[id] {
			return fmt.Errorf("missing mandatory F1CTrafficTransfer IE id=%d", id)
		}
	}
